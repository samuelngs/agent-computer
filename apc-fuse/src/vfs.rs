...
impl HostFs {
    pub fn listen_and_init(port: u32, host_path: &str) -> anyhow::Result<Self> {
        eprintln!("apc-fuse: listening on vsock port {port}");
        let stream = vsock_accept(port)?;
        eprintln!("apc-fuse: host connected");
        ...
        match resp.result {
            FsResult::Ok { .. } => {
                eprintln!("apc-fuse: init ok for {host_path}");
            }
            FsResult::Err { errno } => {
